...
type ServerConfig struct {
	...
	// DeploymentIDHeader lets a trusted caller name the deployment in a header rather than carry it
	// in its token. It applies only where the server chooses to trust it, and only once the caller's
	// token has been validated.
	DeploymentIDHeader DeploymentIDHeaderConfig `yaml:"deployment_id_header" json:"deployment_id_header"`
...
// DeploymentIDHeaderConfig lets a trusted caller name the deployment a request acts for.
//
// A header is caller controlled, so on its own it can never decide which rows a request reaches: any
// caller could name any deployment. What makes it safe here is that it is accepted only from a
// caller that already proved two things - a valid token, and a key only the server and that caller
// know. The key is what distinguishes a trusted intermediary, such as a gateway acting on behalf of
// many tenants, from an ordinary caller who may only ever act for the deployment in its own token.
//
// It is off unless switched on. Turning it off leaves the rest of the deployment resolution exactly
// as it was: the id still comes from the token claim, and only the header is no longer read.
type DeploymentIDHeaderConfig struct {
	// Enabled turns header-supplied deployment ids on. Off by default, because honoring a header is
	// what lets one caller act for tenants other than its own: that has to be asked for, never
	// inherited from a key someone left in a file.
	Enabled bool `yaml:"enabled" json:"enabled"`
	// KeyHeader is the header carrying the shared key. Empty falls back to DefaultDeploymentKeyHeader.
	KeyHeader string `yaml:"key_header" json:"key_header"`
	// Key is the value that header must carry.
	Key string `yaml:"key" json:"key"`
	// IDHeader is the header carrying the deployment id. Empty falls back to DefaultDeploymentIDHeader.
	IDHeader string `yaml:"id_header" json:"id_header"`
}

// Default header names used when the configuration names none.
const (
	DefaultDeploymentKeyHeader = "X-Deployment-Key"
	DefaultDeploymentIDHeader  = "X-Deployment-Id"
)

// Honors reports whether a header may name the deployment. Both halves are required: the switch says
// it is wanted, and the key is what a caller has to prove.
func (c DeploymentIDHeaderConfig) Honors() bool {
	return c.Enabled && strings.TrimSpace(c.Key) != ""
}

// KeyHeaderName returns the header carrying the shared key.
func (c DeploymentIDHeaderConfig) KeyHeaderName() string {
	if strings.TrimSpace(c.KeyHeader) == "" {
		return DefaultDeploymentKeyHeader
	}
	return c.KeyHeader
}

// IDHeaderName returns the header carrying the deployment id.
func (c DeploymentIDHeaderConfig) IDHeaderName() string {
	if strings.TrimSpace(c.IDHeader) == "" {
		return DefaultDeploymentIDHeader
	}
	return c.IDHeader
}

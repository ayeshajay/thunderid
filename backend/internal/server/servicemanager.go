...
func registerServices(mux *http.ServeMux, cacheManager cache.CacheManagerInterface,
	...
	// Secret references in stored configuration are resolved against a provider, and which plane keeps
	// one is the plane's decision: a gateway holds the credentials the configuration applied to it
	// refers to, and a control plane holds none. The store is built here rather than later because a
	// reference met while loading declarative resources must already resolve.
	if deps.SecretStore != nil {
		deps.SecretStore(ctx, mux, configCryptoSvc, runtime.Config.Server.Identifier)
	}
	...
	userService, ouUserResolver, userExporter, err := user.Initialize(
		mux, entityService, ouService, entityTypeService, ouAuthzService, deps.SecretCapturer,
	)
	...
	agentService, agentExporter, err := agent.Initialize(mux, entityService, inboundClientService, ouService,
		roleService, deps.SecretCapturer)
	...
	return &Services{
		...
		ConfigCryptoSvc:        configCryptoSvc,
		JWTService:             jwtService,

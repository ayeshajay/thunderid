...
// VersionsConfig bounds an organization's captured history and what a gateway may be returned to.
//
// A version is a full copy of the organization's configuration, so keeping every one of them grows
// without limit for no benefit: what is useful is the recent past. A version some gateway is still
// running is always kept, whatever this says, because going back must never land on a version that
// has been pruned away.
type VersionsConfig struct {
	// Keep is how many of the organization's newest versions are retained. Zero falls back to
	// DefaultKeepVersions.
	Keep int `yaml:"keep" json:"keep"`
	// RevertToPreviousOnly limits a gateway to stepping back one version at a time rather than
	// jumping to any version in the history. Stepping back to the state immediately before is a
	// decision an operator can reason about; jumping several versions back is one they usually
	// cannot, because everything applied in between goes with it.
	RevertToPreviousOnly bool `yaml:"revert_to_previous_only" json:"revert_to_previous_only"`
}

// DefaultKeepVersions is how many of an organization's versions are retained when none is configured.
const DefaultKeepVersions = 3

// KeepVersions is how many versions to retain, falling back to the default.
func (c VersionsConfig) KeepVersions() int {
	if c.Keep <= 0 {
